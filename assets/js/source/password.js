$(document).ready(function() {
    ...
    let token = k.getUrlParameter('token');
    if (!token) {
        location.href="/";
    }

    // get user info from token
    k.get('PasswordReset', 'token', token, [], function(response) {
    ...
    $("#password-form").submit(function(e) {
        ...
        if (c == p && c.length >= 8 && passreq.validatePassword($("#password"))) {
            ...
            k.put('PasswordReset', 'email', encodeURIComponent(email), {'token':token, 'password':$("#password").val()}, null, [], function(response) {
